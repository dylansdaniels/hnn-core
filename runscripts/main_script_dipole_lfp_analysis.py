...
plt.plot(times[1:],test_comp_L5[1:], label = 'cap + intr + syn')
plt.legend()
plt.xlabel('time [ms]')
...
plt.plot(times[1:],test_comp_I_t_over_gid[1:], label = 'cap + intr + syn')
plt.legend()
plt.xlabel('time [ms]')
...
plt.plot(times[1:],I_cap_intr_over_gid_comp[1:], label = 'capacitive + intrinsic current')
plt.legend()
plt.xlabel('time [ms]')
plt.ylabel('current [nA]')
...
# 2. sum of capacitive + intrinsic components 
...
plt.plot(times[1:], lfp_intr_soma[contact_number,1:], "--", label="cap + intr")  
plt.plot(times[1:], lfp_syn_soma[contact_number,1:], ":", label="syn, L5 soma")  
plt.plot(times[1:], lfp_intr_soma[contact_number,1:] + lfp_syn_soma[contact_number,1:], "--", label="cap + intr + syn")
plt.legend()
plt.xlabel("Time (ms)")
plt.ylabel("LFP (µV)")
...
# plot of all the contacts:
# ALL THE TRACES — agg_i_mem vs (cap + intrinsic + synaptic):
positions = np.asarray(net.rec_arrays["probe1"].positions)
z = positions[:, 2]
...
for c in range(n_contacts):
    ...
    ax.plot(times[1:], traces_comp[c, 1:] + c * offset,
            color="tab:red", lw=0.8, alpha=0.8,
            label="cap + intrinsic + synaptic" if c == 0 else None)

ax.set_yticks([c * offset for c in range(n_contacts)])
ax.set_yticklabels([f"{int(z[c])} µm" for c in range(n_contacts)], fontsize=9)
ax.set_xlabel("Time (ms)")
ax.set_ylabel("Depth z")
ax.set_title("LFP per contact — agg_i_mem vs cap + intrinsic + synaptic")
...
scale_uV = 30.0                     # change to taste (1, 2, 5, 10, ...)
bar_height_visual = scale_uV * gain   # how tall the bar should be on screen
